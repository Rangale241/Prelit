...
# Testing Output
st.write("selected:", selected[:10])
st.write("translated_lemmas:", translated_lemmas[:10])
...
#st.text(tsv_data[:200])
#if col1.download_button(f":primary[Download Flashcards]", data=formatted_flashcards,
      #                  file_name="anki_export.tsv",
     #                   mime="text/tab-separated-values"):
    #st.text("Exporting Flashcards")

if col1.button(":primary[Display Flashcards]"):
    st.text_area("Flashcards Preview", value=formatted_flashcards, height=200)
...
if st.button(":primary[📂 Open Export Folder]"):
    st.write("Button Temporarily Unavailable")
